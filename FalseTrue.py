...
def fix_extension(file_path):
    """Fix unknown extensions by renaming to zip or rar"""
    name, ext = os.path.splitext(file_path)
    ext = ext.lower()
    if ext not in [".zip", ".rar", ".7z", ".csv"]:
        # If the file has .ac or .ab etc., try to rename
...
def extract_file(file_path, extract_dir):
    """Extract zip/rar/7z files recursively"""
...
def load_csvs_recursive(root_dir):
    """Walk through folders and load all CSV paths"""
    csv_files = []
...
def search_csvs(keyword):
    """Search keyword in all CSVs recursively"""
    results = []
